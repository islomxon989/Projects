...
class Game {
  ...
  def checkForBaseDamage(): Unit = {
    for (client <- players.values) {
      val UBL: PhysicsVector = new PhysicsVector(level.base.x + 0.5, level.base.y + 0.5)
      val UL: PhysicsVector = client.location
      val DF: Double = UL.distance2d(UBL)
      if (playerSize > DF){
        baseHealth = baseHealth - 1
        client.location = startingVector()
      }
    }
  }
  ...
  def checkForPlayerHits(): Unit = {
    for (client <- players.values) {
      for (p <- projectiles){
        val PL: PhysicsVector = new PhysicsVector(p.location.x, p.location.y)
        val UL: PhysicsVector = client.location
        val DF: Double = UL.distance2d(PL)
        if (playerSize > DF){
          client.location = startingVector()
          p.destroy()
        }
